...
uint8_t sync_flag = 0;
uint32_t tick_diff_threshold = 1000; //待定？
uint8_t window_diff = 10; //窗口误差


uint8_t slave_num;				// 终端数目
uint32_t interval_period;       // 间隔周期 = (终端发送周期/终端数目)
uint32_t sync_start = 50000;			   // 同步开始时间 > 最大采集周期(所有终端)
...
void user_main(void)
{
	...
	uint32_t send_period = 24000;		// 终端发送周期 = 最小采集周期(所有终端)
	uint32_t recv_period = (24000 / slave_num) - window_diff; // 中心站接受周期 =间隔周期 -误差值(待定)
	
	interval_period = (24000 / slave_num);
	/*
		各个终端的采样时间
		终端A：（10min、15min、25min）
		终端B：（20min、20min、25min）
		终端C：（30min、30min、30min）
		终端D：（10min、20min、30min）
	*/
	...
	get_distance_thread_data->time = 10000; //ms为单位
	...
	getXYZAxisAccelerationsThread_data->time = 30000;
	...
	master_rx_data->time = recv_period;    //200s               
