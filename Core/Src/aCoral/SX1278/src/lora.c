...
{   
    ...
    {    
        ...
        */
        ...
        if (rx_cmd.slave_device_id == (uint8_t)0xffu)
        {
...
void send_data()
{
    ...
    if (tx_done)
    {
        ...
        acoral_print("terminal:%d sending\r\n",slave_device_id);
        tx_done = 0;

        // 开始等待中心站应答信号，将LoRa切换为接收模式，若校验值不一致则重传数据包三次，三次之后放弃此次发送。
        for (int i = 0; i < 2; i++)
        {
            if (slave_rx_resend() == 1)
            {
                break;
            }
            if(i==1)
...
/**
 * @brief 终端接收服务函数slave_rx_resend，需重传
 * @param None  
 * @return 0，应答数据包不一致重新发送数据；1，应答数据包一致更改周期参数
 */
uint8_t slave_rx_resend()
{  
    uint8_t timeout = 10;
    Radio->StartRx();
    ...
    if(rx_done)
    {
        ...
        if ((rx_cmd.master_id == master_id) && (rx_cmd.slave_device_id == slave_device_id)) 
        {
            ...
            if (rx_cmd.change_period_significant != 0)
            {
                ...
                cur_period.period_significant = rx_cmd.change_period_significant;
